            ...
            if ($z -gt 2) {
                ...
                if ($instance[$z] -ne '') {
                    ...
                    $button.Add_Click({
                        ...
                        write-host $($config.$type[$posArray].$rdsType);
                        write-host $posArray;
                        ...
                        wt --window 0 -p "Windows Powershell" `
                            ...
                            powershell `
                            -noExit `
                            "$($config.$type[$posArray].command) $($config.$type[$posArray].profile) $($data.port_number) $($data.local_port_number) $($config.$type[$posArray].$rdsType) $($data.instance_id)";

                        # Sleep because it takes a while for the listener to spawn
                        Start-Sleep -Seconds 3

                        # Generate token [text output only in hidden debug window!]
                        $token = aws rds generate-db-auth-token --hostname $($config.$type[$posArray].$rdsType) --port $($data.port_number) --username $($config.$type[$posArray].username) --region=eu-central-1 --profile $($config.$type[$posArray].profile);
                        $text = "`n";
